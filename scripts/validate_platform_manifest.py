...
def check_structure(manifest: dict, report: Report) -> None:
    ...
    if set(actual_evidence) != set(expected_evidence) or len(
        evidence_rows or []
    ) != len(expected_evidence):
        report.error(
            f"qualification_evidence IDs must be exactly {sorted(expected_evidence)!r}"
        )
        ...
        if expected is not None:
            for field in ("kind", "source_component", "repository", "path"):
                if expected.get(field) != evidence.get(field):
                    report.error(
                        f"qualification evidence {evidence.get('id')!r} {field} drift"
                    )
            if expected.get("source_component") is None and evidence.get(
                "url"
            ) != expected.get("url"):
                report.error(f"qualification evidence {evidence.get('id')!r} URL drift")
    ...
    for name, schema in (schemas or {}).items():
        ...
        if (
            ...
            schema.get("minimum") != accepted[0]
            or schema.get("maximum") != accepted[-1]
        ):
...
def compare_component_to_pypi(
    ...
    * any filename/URL/digest mismatch -> ERROR, always.
    ...
    declared = {artifact.get("filename") for artifact in component.get("artifacts", [])}
...
def check_schema_sources(manifest: dict, report: Report, require_network: bool) -> None:
    try:
...
def main() -> int:
    ...
    if not args.offline:
        ...
        check_source_provenance(manifest, report, require_network=args.require_network)
        check_schema_sources(manifest, report, require_network=args.require_network)
