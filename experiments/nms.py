...
def sliding_window(image: Image.Image, step_ratio: float, window_ratio: tuple[float, float]) -> list[tuple[Image.Image, int, int]]:
    ...
    window_size = (int(image.width * window_ratio[0]), int(image.height * window_ratio[1]))
    patches = []
    ...
    for y in range(0, image.height, step_size[1]):
        for x in range(0, image.width, step_size[0]):
            ...
            if x + window_size[0] > image.width:
                x = image.width - window_size[0]  # Adjust x to start earlier so the patch fits
            if y + window_size[1] > image.height:
                y = image.height - window_size[1]  # Adjust y to start earlier so the patch fits
...
def combine_and_nms(image: Image.Image, patches_boxes: list[tuple[list[dict[str, float]], int, int]]) -> list[dict[str, float]]:
    ...
    boxes = np.array([[box["left"], box["top"], box["left"] + box["width"], box["top"] + box["height"]] for box in bounding_boxes])
    scores = np.ones(len(boxes))
    
    indices = cv2.dnn.NMSBoxes(boxes.tolist(), scores.tolist(), score_threshold=0.1, nms_threshold=0.9)
    print(f'{indices=}')
