...
def test_openai_completion_provider():
    ...
    gpt_4_chat_response = GPTCompletionProvider.infer("gpt-4","What is your maximum context size?")
    
    gpt_3_turbo_chat_response = GPTCompletionProvider.infer("gpt-3.5-turbo","What day is it today?")

    davinci_completion_response = GPTCompletionProvider.infer("davinci","Today is ")

    assert (len(gpt_3_turbo_chat_response) > 0 and len(gpt_4_chat_response) > 0 and len(davinci_completion_response) > 0)

def test_huggingface_completion_provider():

    inference_output = GenericHuggingFaceProvider.infer("What is the next number in the series 1, 3, 5, 7 ",
    "gpt2-medium","text-generation",trust_remote_code=True)
