...
class RWKVReplayStrategyMixin:
    ...
    def get_response(self,model=MODEL, instruction=None, task_description=None, input=None, parameters=None):
        ...
        if USE_MODAL:
            ...
            response = function_call.call(model=model, task_description=task_description, input=input, parameters=parameters)
            logger.debug(f"response=\n{response}")
        else:
            response = run_RWKV(model=MODEL, task_description=task_description, input=input, parameters=parameters)
        return response