...
class RWKVReplayStrategyMixin:
    ...
    def get_response(self,model=MODEL, instruction=None, task_description=None, input=None, parameters=None):
        ...
        if USE_MODAL:
            ...
            response = function_call.call(model_number=model, task_description=task_description, input=input, parameters=parameters, use_cuda=True)
            logger.debug(f"response=\n{response}")
        else:
            response = run_RWKV(model_number=model, task_description=task_description, input=input, parameters=parameters, use_cuda=False)
        return response