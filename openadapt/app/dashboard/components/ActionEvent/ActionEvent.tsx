...
export const ActionEvent = ({
    ...
    const { ref: screenshotRef, hovered: hoveredOverScreenshot } =
    ...
    const imageSrc =
        (hoveredOverScreenshot ? event.screenshot : event.screenshot) || '' // change to event.diff to show diff
    ...
    const displayChild =
        event.children && event.children.length > 0 && event.children[0].id
    ...
    if (displayChild) {
        content = (
            <Accordion.Item value={event.id.toString()}>
                ...
                {displayChild &&
                    values.includes(event.id.toString()) &&
                    event.children && (
