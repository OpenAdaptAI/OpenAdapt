    ...
    return (
        <form onSubmit={form.onSubmit(_onSubmit)}>
            <Grid>
                <Grid.Col span={6}>
                    <Checkbox
                        label="Scrubbing Enabled"
                        {...form.getInputProps('SCRUB_ENABLED')}
                        checked={form.values.SCRUB_ENABLED}
                    />
                </Grid.Col>
                <Grid.Col span={6}>
                    <TextInput
                        label="Scrubbing character"
                        placeholder="Scrubbing character"
                        {...form.getInputProps('SCRUB_CHAR')}
                    />
                </Grid.Col>
                <Grid.Col span={6}>
                    <TextInput
                        label="Scrubbing language"
                        placeholder="Scrubbing language"
                        {...form.getInputProps('SCRUB_LANGUAGE')}
                    />
                </Grid.Col>
                <Grid.Col span={6}>
                    <ColorInput
                        label="Scrubbing fill color"
                        placeholder="Scrubbing fill color"
                        {...form.getInputProps('SCRUB_FILL_COLOR')}
                    />
                </Grid.Col>
            </Grid>
...
function getSettingsCopy(settings: Record<string, string>) {
    return JSON.parse(
        JSON.stringify({
            ...settings,
            SCRUB_FILL_COLOR: convertBGRIntToHex(
                parseInt(settings.SCRUB_FILL_COLOR)
            ),
        })
    )
}
