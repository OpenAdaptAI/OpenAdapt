...
export default function Recordings() {
    ...
    const [scrubbingProviders, setScrubbingProviders] = useState<Record<string, string>>({});
    const [scrubbingStatus, setScrubbingStatus] = useState<ScrubbingStatus>(ScrubbingStatus.UNKNOWN);
    ...
    function scrubRecording(values: { recordingId: string, providerId: string}) {
    ...
    return (
        <Container size="sm">
            {scrubbingStatus === ScrubbingStatus.UNKNOWN ? (
                ...
                <ScrubbingUpdates data={scrubbingUpdate} resetScrubbingStatus={resetScrubbingStatus} />
            ) : (
                <form onSubmit={scrubbingForm.onSubmit(scrubRecording)}>
                    <Grid>
                        ...
                        <Grid.Col span={6}>
                            <Select
                                data={Object.entries(scrubbingProviders).map(([id, name]) => ({
                                    value: id,
                                    label: name,
                                }))}
                                size="sm"
