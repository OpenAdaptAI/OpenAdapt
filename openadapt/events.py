...
def get_events(
    ...
    if process:
        while True:
            ...
            action_events, window_events, screenshots, browser_events = process_events(
                action_events, window_events, screenshots, browser_events
            )
            if (
                len(action_events) == num_action_events and
                len(window_events) == num_window_events and
                len(screenshots) == num_screenshots and
                len(browser_events) == num_browser_events
            ):
                break
            num_process_iters += 1
            num_action_events = len(action_events)
...
def process_events(action_events, window_events, screenshots, browser_events):
    # for debugging
    _action_events = action_events
    _window_events = window_events
    _screenshots = screenshots
    _browser_events = browser_events
